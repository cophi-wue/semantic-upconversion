...
To counter this issue, we created a stratified sample. From 25 novels (1071 segments) of the [test corpus](dataset.qmd) from different series and publishers all non-chapter chunks were taken (134 in total). To reach 200 text chunks the remaining 67 segments were filled with randomly taken chapters from the same pool of novels.
...
To resolve these boundary conflicts, we restructured the vocabulary into a two-tier taxonomy [link to taxonomy page](guidelines.qmd#label-taxonomy) of primary and secondary labels and restricted the LLM evaluation to the primary labels. Additionally, we benchmarked state-of-the-art open-weight models.
