...
In the first evaluation phase, we tested five small open-weight models (≤120B parameters) to test whether smaller architectures could deliver sufficient classification performance while minimizing computational overhead. The models were prompted to classify each segment of the gold dataset into an 11-class taxonomy shown in the distribution table above.
...
To resolve these boundary conflicts, we restructured the vocabulary into a two-tier [taxonomy](guidelines.qmd#label-taxonomy) of primary and secondary labels and restricted the LLM evaluation to the primary labels. Additionally, we benchmarked state-of-the-art open-weight models from top leaderboard rankings to evaluate the performance gains of significantly larger parameter scales compared to the smaller models from Round 1.
